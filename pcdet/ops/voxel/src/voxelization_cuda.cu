...
void dynamic_voxelize_gpu(const at::Tensor& points, at::Tensor& coors,
  ...
  const int grid_x = ceil((coors_x_max - coors_x_min) / voxel_x);
  const int grid_y = ceil((coors_y_max - coors_y_min) / voxel_y);
  const int grid_z = ceil((coors_z_max - coors_z_min) / voxel_z);
