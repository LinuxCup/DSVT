...
class VoxelizationImpe(torch.autograd.Function):
    @staticmethod
    def forward(ctx,src, points_num):
        ...
        point_features_data = torch.zeros((150000, 10))
        point_index_in_voxel_data = torch.zeros((150000, 10))
        # test = torch.zeros((150000, 10))
        return point_features_data, point_index_in_voxel_data
    ...
    @staticmethod
    def symbolic(g, src, points_num):
        return g.op("VoxelizationPlugin",src, points_num)
...
class VFETRT(nn.Module):
    ...
    def forward(
        ...
        points_num = torch.tensor(164883).unsqueeze(dim=0).cuda()
        # pdb.set_trace()
        point_features_data, point_index_in_voxel_data = voxelization(points, points_num)
        return point_features_data, point_index_in_voxel_data
        pdb.set_trace()
