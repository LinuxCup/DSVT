...
# cfg_file = "/home/zhenghu/DeepLearning/DSVT/tools/cfgs/dsvt_models/dsvt_plain_1f_onestage_zhito.yaml"
    ...
    def forward(
        ...
        for block_id in range(4):
            ...
            for shift_id in range(2):
                ...
                coors_in_win = coors_in_wins[shift_id]
            
                embed_layer = self.posembed_layers[0][block_id][shift_id]

                # if (0 == shift_id):
                #     win_x, win_y, win_z = window_shape
                # else:
                #     win_x, win_y, win_z = [e *2 for e in window_shape]
...
with torch.no_grad():
    ...
    input_shapes = {
        ...
        "pos_embed_tensor": {
            "min_shape": [4, 2, 24629, 128],
            "opt_shape": [4, 2, 24629, 128],
            "max_shape": [4, 2, 24629, 128],
        },
    }
    ...
    base_name = "combine3modules_dynamic_shape_noscatter_waymo"
    ts_path = f"{base_name}.ts"
    onnx_path = f"{base_name}.onnx"
    ...
    # pdb.set_trace()
    torch.onnx.export(
        combine3modules, spatial_features, onnx_path,
        input_names=['input'],
    ...
    base_name = "pos_enbendding_waymo"
    ts_path = f"{base_name}.ts"
    onnx_path = f"{base_name}.onnx"

    pos_enbendding_inputs = (torch.stack([pillars_coors_in_win_shift0, pillars_coors_in_win_shift1]))
    input_names = ["coors_in_win"]
